...
(function($){
	...
	$('a.menu-link[href*="#"]:not([href="#"])').on("click", function() {
		if (location.pathname.replace(/^\//, '') === this.pathname.replace(/^\//, '') && location.hostname === this.hostname) {
			...
			if (toHash.length) {
				$('html, body').animate({
					scrollTop: (toHash.offset().top+100 - nbar)
				}, 1000, "easeInOutExpo");
				return false;
			}
		}
		//alert(this.pathname+'|'+this.hostname);
	});
	...
	if ($count_token.length > 0 ) {
		$count_token.each(function() {
			...
			$self.countdown(datetime).on('update.countdown', function(event) {
				$(this).html(event.strftime('' + '<div class="col"><span class="countdown-time countdown-time-first">%D</span><span class="countdown-text">Days<span>ays</span></span></div>' + '<div class="col"><span class="countdown-time">%H</span><span class="countdown-text">Hours<span>ours</span></span></div>' + '<div class="col"><span class="countdown-time">%M</span><span class="countdown-text">Minutes<span>inutes<span></span></div>' + '<div class="col"><span class="countdown-time countdown-time-last">%S</span><span class="countdown-text">Seconds<span>econds</span></span></div>'));
			});
		});
	}
