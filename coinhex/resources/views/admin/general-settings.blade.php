        ...
        <form name="frm" action="{{ route('save-general-settings') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="saved_logo_id" value="<?php if(!empty($settings['site_logo_id'])){ echo $settings['site_logo_id']; }?>">
			            ...
			            <div class="form-group">
                <label>{{ _tr('Whitepaper') }}</label>
                <input type="file" name="whitepaper">
                <?php 
                if(!empty($settings['whitepaper_id']) && $settings['whitepaper_id'] > 0){
					?>
                    <br/>
                    <a href="<?php echo get_recource_url($settings['whitepaper_id']); ?>" target="_blank">View File</a>
                    <br/>
                    <input type="checkbox" name="remove_whitepaper" id="remove_whitepaper" value="yes">
                    {{ _tr('Remove Whitepaper') }}
					<?php
                }
                ?>
                <input type="hidden" name="saved_whitepaper_id" value="<?php if(!empty($settings['saved_whitepaper_id'])){ echo $settings['saved_whitepaper_id']; }?>">
            </div>
			
			<div class="form-group">
                <label>{{ _tr('Privacy Policy') }}</label>
                <input type="file" name="privacy_policy_file">
                <?php 
                if(!empty($settings['privacy_policy']) && $settings['privacy_policy'] > 0){
					?>
                    <br/>
                    <a href="<?php echo get_recource_url($settings['privacy_policy']); ?>" target="_blank">View File</a>
                    <br/>
                    <input type="checkbox" name="remove_privacy_policy" id="remove_privacy_policy" value="yes">
                    {{ _tr('Remove Privacy Policy') }}
					<?php
                }
                ?>
                <input type="hidden" name="privacy_policy" value="<?php if(!empty($settings['privacy_policy'])){ echo $settings['privacy_policy']; }?>">
            </div>

            <div class="form-group">
                <label>{{ _tr('Terms & Conditions') }}</label>
                <input type="file" name="terms_conditions_file">
                <?php 
                if(!empty($settings['terms_conditions']) && $settings['terms_conditions'] > 0){
					?>
                    <br/>
                    <a href="<?php echo get_recource_url($settings['terms_conditions']); ?>" target="_blank">View File</a>
                    <br/>
                    <input type="checkbox" name="remove_terms_conditions" id="remove_terms_conditions" value="yes">
                    {{ _tr('Remove Terms & Conditions') }}
					<?php
                }
                ?>
                <input type="hidden" name="terms_conditions" value="<?php if(!empty($settings['terms_conditions'])){ echo $settings['terms_conditions']; }?>">
            </div>
			...
			<div class="form-group">
                <label>{{ _tr('Admin Wallet Address (SOL)') }}</label>
                <input class="form-control" type="text" name="admin_wallet_address_sol" id="admin_wallet_address_sol" value="<?php if(!empty($settings['admin_wallet_address_sol'])){ echo $settings['admin_wallet_address_sol']; }?>">
            </div>
