	...
	<div class="user-panel">
		...
		<?php 
			...
			if($errors->has('token_address')){
				...
				<li class="nav-item">
					<a class="nav-link " data-toggle="tab" href="#change-password">{{ __t('Change Password') }}</a>
				</li>
		<?php }
			else if($errors->has('cpwd') || $errors->has('npwd') || $errors->has('npwd_confirmation') ){
				$active3 = 'show active';
				?>
				<li class="nav-item">
					<a class="nav-link" data-toggle="tab" href="#personal-data">{{ __t('Personal Data') }}</a>
				...
				<li class="nav-item">
					<a class="nav-link active" data-toggle="tab" href="#change-password">{{ __t('Change Password') }}</a>
				</li>
		<?php }else{ 
			$active = 'show active';
		?>
		...
		<?php } ?>
			...
			<div class="tab-content" id="myTabContent">
				...
				<div class="tab-pane fade <?php echo $active2; ?>" id="wallet-address">
					<form action="{{ route('account-post') }}" method="post" name="account-post" id="account-post">
						...
						<div class="input-item input-with-label">
							...
							<input class="input-bordered" type="text" id="wallet_address1" name="token_address" value="<?php  if(isset($account_detail) && $account_detail->wallet_address !=""){ echo $account_detail->wallet_address; }else { echo old('token_address');} ?>" maxlength="60" >
							<span class="input-note">{{ __t('Note: To receive tokens. Address should be BEP20 BNB Wallet. Make sure your wallet has private key!') }}</span>
							@if($errors->has('token_address'))
				...
				<div class="tab-pane fade <?php echo $active3; ?>" id="change-password">
					<form action="{{ route('user_save-cng-pwd') }}" method="post" name="account-post" id="account-post">
						{{ csrf_field() }}
						<div class="input-item input-with-label">
							<label for="cpwd" class="input-item-label">{{ __t('Current Password') }}  </label>
							<input type="password" name="cpwd" id="cpwd" class="input-bordered" value="{{ old('cpwd') }}" autocomplete="off">
						
							@if($errors->has('cpwd'))
								<br><span style="color:RED;"><small>{{ __t($errors->first('cpwd')) }}</small></span>
							@endif
							
						</div><!-- .input-item -->
						<div class="input-item input-with-label">
							<label for="npwd" class="input-item-label">{{ __t('New Password') }}  </label>
							<input type="password" name="npwd" id="npwd" class="input-bordered" value="{{ old('npwd') }}" autocomplete="off">
						
							@if($errors->has('npwd'))
							<br><span style="color:RED;"><small>{{ __t($errors->first('npwd')) }}</small></span>
							@endif
						</div>
						
						<div class="input-item input-with-label">
							<label for="rpwd" class="input-item-label">{{ __t('Confirm Password') }}  </label>
							<input type="password" name="npwd_confirmation" id="rpwd" class="input-bordered" value="{{ old('npwd_confirmation') }}" autocomplete="off">
						
							@if($errors->has('confirm_password'))
							<br><span style="color:RED;"><small>{{ __t($errors->first('confirm_password')) }}</small></span>
							@endif
						</div>
