                    ...
                    <div class="user-panel">
						...
						<?php if(isset($kyc) > 0 && !empty($kyc) && $kyc->status!=0){ ?>
							...
							<div class="status status-thank">
								...
								<span class="status-text ky">{{ __t('Thank you! You have completed the process for your Identity verification.') }}</span>
							...
							<?php }else if($kyc->status==3){ // information missing ?>
								...
								<div class="status status-warnning">
									...
									<span class="status-text ky">{{ __t('We found some information missing in application.') }}</span>
							...
							<?php }else if($kyc->status==2){ // rejected ?>
								...
								<div class="status status-canceled">
									...
									<span class="status-text ky">{{ __t('Your application rejected by admin.') }}</span>
							...
							<?php }else if($kyc->status==1){ // processing ?>
								 ...
								 <div class="status status-process">
									...
									<span class="status-text ky">{{ __t('Your Application under Process for Verification.') }}</span>
                        ...
                        <div class="status status-empty">
							...
							<?php if(isset($kyc) > 0 && !empty($kyc) && $kyc->status==0){ ?>
							<span class="status-text ky">{{ __t('You have submitted your AML/KYC Application - status is pending') }} </span>
                            <a href="{{ route('kyc-application') }}" class="btn btn-primary">{{ __t('CLick to update') }}</a>
							<?php }else{ ?>
                            <span class="status-text ky">{{ __t('You have not submitted your AML/KYC Application') }}</span>
